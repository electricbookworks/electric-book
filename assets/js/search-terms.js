...
function isSearchPage() {
    'use strict';
    var searchPageCheck = document.querySelector('.search-page, .content form.search');
    if (searchPageCheck) {
        return true;
    } else {
        return false;
...
function ebSearchTermsOnPage() {
    ...
    if (searchTerms) {
        ...
        if (searchTerms.length && searchTerms.length > 0) {
            ...
            var headingPhrase = locales[pageLanguage].search['results-for-singular'];
            ...
            searchTerms.forEach(function (term) {
                ...
                var potentialFigureAncestor = term.closest('.figure[id]');
                ...
                if (parentTextElement && links.indexOf(parentTextElement) === -1) {
                    ...
                    if (parentTextElement.id) {
                        ...
                        // Truncate the text
                        var text = ebTruncatedString(parentTextElement.innerText, 60, ' …');
...
function ebPrepareSearchTermsOnPage() {
    ...
    var searchTermsObserver = new MutationObserver(function (mutations) {

        var readyForSearchTerms = false;
